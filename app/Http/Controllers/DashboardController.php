...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // --- 2. LOGIC MODE: DELIVERY (Deep Analytics) rill ---
        $deliveryPerformance = 0;
        $deliveryTrend = [];
        $customerShipments = [];

        if ($mode == 'delivery') {
            // A. Hitung % Performa (Total Qty Terkirim vs Total Qty Order di PO)
            $totalOrdered = DB::table('purchase_order_items')->sum('qty') ?? 0;
            $totalSent = DB::table('deliveries')->sum('qty_delivery') ?? 0;
            $deliveryPerformance = ($totalOrdered > 0) ? round(($totalSent / $totalOrdered) * 100, 1) : 0;

            // B. Tren Pengiriman 30 Hari Terakhir (Harian/Bulanan)
            ...
            // C. Rincian Per Customer (Siapa yang paling banyak dikirim)
        ...
        // --- 3. LOGIC MODE: SUMMARY (Bar Chart & Shortage) ---
        ...
        // Data 7 hari terakhir buat mini-chart (opsional rill)
        $delvDates = []; $delvQtys = [];
        for ($i = 6; $i >= 0; $i--) {
        ...
        return view('dashboard', compact(
            ...
            'delvDates', 'delvQtys', 'mode', 
            'deliveryPerformance', 'deliveryTrend', 'customerShipments'
        ));
    }
}