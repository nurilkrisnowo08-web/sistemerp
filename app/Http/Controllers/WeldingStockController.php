    ...
    public function index(Request $request)
    {
        ...
        $inventoryWelding = DB::table('finished_goods')
            ->select(
                ...
                'part_name',
                'customer',
                'welding_stock as live_stock' // Saldo real-time di database
            )
            ->where(function($q) {
            ...
            ->map(function($item) use ($date) {
                ...
                // 1. ✨ FIX IN: Hitung Masuk dari Produksi Stamping hari ini
                // Pastikan ProduksiController mencatat ke production_logs saat stamping selesai
                $in_stamping = DB::table('production_logs')
                    ->whereRaw("REPLACE(part_no, ' ', '') = ?", [$cleanPart])
                    ->whereDate('created_at', $date)
                    ->sum('qty');

                // 2. ✨ FIX OUT: Hitung pengambilan ke meja las hari ini
                // Diambil dari tabel welding_batches yang dibuat saat klik tombol 'TAKE' (Deploy)
                ...
                // Rumus: Stok Sekarang - Masuk Hari Ini + Keluar Hari Ini
                $item->init = $item->live_stock - $in_stamping + $out_welding;
                $item->in_s = $in_stamping;
                $item->out = $out_welding;
                
                // 4. Hitung Sisa Jalan (RUN) per 50 pcs (Standar Pallet)
