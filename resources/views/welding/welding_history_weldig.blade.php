...
<style>
    ...
    .remark-text { font-size: 11px; color: #64748b; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
</style>
...
<div class="container-fluid py-4 px-4">
    ...
    @php
        ...
        $grandTotal = (float)$totalOk + (float)$totalNg;
        $avgYield = $grandTotal > 0 ? ($totalOk / $grandTotal) * 100 : 0;
    @endphp
    <div class="row mb-4">
        <div class="col-md-3"><div class="stat-card"><div class="stat-label">Total Inspections</div><div class="stat-value text-primary">{{ $totalBatch }}</div></div></div>
        <div class="col-md-3"><div class="stat-card border-left-success" style="border-left: 5px solid var(--brand-success) !important;"><div class="stat-label">Average Yield</div><div class="stat-value text-success">{{ number_format($avgYield, 1) }}%</div></div></div>
        <div class="col-md-3"><div class="stat-card"><div class="stat-label">Total OK PCS</div><div class="stat-value">{{ number_format($totalOk) }}</div></div></div>
        <div class="col-md-3"><div class="stat-card border-left-danger" style="border-left: 5px solid var(--brand-danger) !important;"><div class="stat-label">Total NG PCS</div><div class="stat-value text-danger">{{ number_format($totalNg) }}</div></div></div>
    </div>
            ...
            <table class="table table-history mb-0 text-center">
                <thead>
                    <tr>
                        ...
                        <th>Yield</th>
                        <th>Inspector</th>
                        <th class="text-left">Reason / Remark</th> {{-- ✨ TAMBAHAN KOLOM --}}
                    </tr>
                </thead>
                <tbody class="bg-white">
                    ...
                    <tr onclick="showDetail({{ json_encode($h) }})">
                        <td class="text-left pl-4 text-muted small">
                            {{ $h->qc_at ? \Carbon\Carbon::parse($h->qc_at)->format('d/m/Y H:i') : \Carbon\Carbon::parse($h->updated_at)->format('d/m/Y H:i') }}
                        </td>
                        <td class="font-weight-bold text-primary font-mono">{{ $h->no_produksi_stamping }}</td>
                        <td class="text-dark font-weight-bold text-left text-uppercase">{{ $h->part_no }}</td>
                        ...
                        <td class="text-uppercase small font-weight-bold text-muted">
                            {{ $h->qc_by ?: 'STATION_ADMIN' }}
                        </td>
                        <td class="text-left">
                            <div class="remark-text">{{ $h->keterangan ?: '-' }}</div> {{-- ✨ TAMBAHAN DATA --}}
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="8" class="py-5 text-muted italic">No historical data available.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="modal-content shadow-lg">
            ...
            <div class="modal-body p-4">
                <div class="text-center mb-4">
                    ...
                    <span class="badge badge-success px-4 py-2 rounded-pill font-weight-bold">VERIFIED PRODUCTION</span>
                </div>
                <div class="row">
                    <div class="col-6 mb-3 border-right"><div class="detail-label">Batch No</div><div class="detail-value text-primary font-mono" id="det-batch"></div></div>
                    <div class="col-6 mb-3"><div class="detail-label">Verification Date</div><div class="detail-value" id="det-date"></div></div>
                    <div class="col-12 mb-3"><div class="detail-label">Part Identification</div><div class="detail-value h5 font-weight-bold" id="det-part"></div></div>
                    <div class="col-4 mb-3 border-right"><div class="detail-label">Qty OK</div><div class="detail-value text-success h4" id="det-ok"></div></div>
                    <div class="col-4 mb-3 border-right"><div class="detail-label">Qty NG</div><div class="detail-value text-danger h4" id="det-ng"></div></div>
                    <div class="col-4 mb-3"><div class="detail-label">Inspector</div><div class="detail-value" id="det-by"></div></div>
...
<script>
    ...
    function showDetail(data) {
        ...
        document.getElementById('det-date').innerText = data.qc_at || data.updated_at;
        ...
        document.getElementById('det-by').innerText = data.qc_by || 'STATION_ADMIN';
