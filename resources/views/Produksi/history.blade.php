...
<style>
    ...
    /* Stats Cards */
    .stat-card { background: #fff; border-radius: 15px; padding: 20px; border: 1px solid #e3e6f0; transition: 0.3s; box-shadow: 0 4px 10px rgba(0,0,0,0.03); height: 100%; }
    ...
    .stat-value { font-family: 'Orbitron'; font-size: 20px; font-weight: 800; color: #2d3436; }
    ...
    .table-hud thead th { background: #f8f9fc; color: var(--ind-steel); font-size: 10px; text-transform: uppercase; letter-spacing: 1px; padding: 18px; border-bottom: 2px solid #eaecf4; }
    .row-clickable { cursor: pointer; transition: 0.2s; }
    .row-clickable:hover { background-color: #f8faff !important; }
    .id-tag { font-family: 'JetBrains Mono'; font-size: 11px; font-weight: 700; color: var(--ind-steel); }
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 px-2">
        <div>
            <h3 class="industrial-header font-weight-bold text-dark mb-0">PRODUCTION_HISTORY <span class="text-primary">v4.5</span></h3>
            <p class="text-muted small font-weight-bold uppercase mb-0">Audit Log // Traceability System</p>
        </div>
    ...
    <div class="row mb-4">
        <div class="col-md-3 mb-3">
            <div class="stat-card border-left border-primary" style="border-left-width: 5px !important;">
                <div class="stat-label">Material Input (Take)</div>
                <div class="stat-value text-primary">{{ number_format($totalAmbil) }} <small style="font-size: 10px;">PCS</small></div>
            </div>
        </div>
        <div class="col-md-3 mb-3">
        ...
        <div class="col-md-2 mb-3">
            <div class="stat-card border-left border-warning" style="border-left-width: 5px !important;">
                <div class="stat-label">Yield Accuracy</div>
                <div class="stat-value text-dark">{{ number_format($avgYield, 1) }}%</div>
            </div>
        </div>
        <div class="col-md-2 mb-3">
            <div class="stat-card bg-dark text-white">
                <div class="stat-label text-white-50">Executed Batches</div>
                <div class="stat-value text-white">{{ $history->count() }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="terminal-card p-4 mb-4">
        <h6 class="font-weight-bold text-muted small uppercase mb-4 tracking-widest"><i class="fas fa-chart-area mr-2"></i> Quality Stability Trend (Last 15 Batches)</h6>
            ...
            <table class="table table-hud mb-0 text-center">
                <thead>
                    <tr>
                        ...
                        <th class="bg-light">AMBIL (PCS)</th>
                        <th>OK (PCS)</th>
                        <th>NG (PCS)</th>
                        <th>Yield</th>
                ...
                <tbody id="historyLogBody">
                    ...
                    @php 
                        ...
                        $batchAmbil = (float)$h->qty_ambil_pcs;
                    ...
                    <tr class="row-clickable" onclick="showDetail({{ json_encode($h) }})">
                        ...
                        <td class="bg-light font-weight-bold font-mono">{{ number_format($batchAmbil) }}</td>
                        ...
                        <td><b style="color: {{ $color }}; font-size: 14px; font-family: 'JetBrains Mono';">{{ number_format($yield, 1) }}%</b></td>
                        <td class="text-left">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content shadow-lg border-0" style="border-radius: 20px; overflow: hidden;">
            ...
            <div class="modal-body p-4 text-left bg-white">
                ...
                {{-- ✨ PERBANDINGAN AMBIL VS HASIL ✨ --}}
                <div class="p-3 bg-dark text-white rounded-lg mb-3 shadow-inner">
                    <div class="d-flex justify-content-between align-items-center">
                        <small class="uppercase font-weight-bold text-info">Material Input (Take)</small>
                        <b class="h5 mb-0 font-mono" id="det-ambil">0</b>
                    </div>
                </div>

                <div class="row mb-3 text-center small font-weight-bold">
                    <div class="col-6 pr-1"><div class="p-3 bg-light rounded-lg border-bottom border-primary"><small class="text-muted d-block uppercase" style="font-size: 8px;">Passed Good</small><b class="text-success h5 font-mono" id="det-ok">0</b></div></div>
                    <div class="col-6 pl-1"><div class="p-3 bg-light rounded-lg border-bottom border-danger"><small class="text-muted d-block uppercase" style="font-size: 8px;">Total Reject</small><b class="text-danger h5 font-mono" id="det-ng">0</b></div></div>
                </div>

                <div class="mb-3">
                    <h6 class="font-weight-bold text-danger small mb-2 uppercase border-bottom pb-2">Reject Breakdown (Specific Defects)</h6>
                    <div id="det-ng-list" class="ng-detail-box"></div>
                </div>
...
<script>
    ...
    function showDetail(h) {
        ...
        const yieldVal = totalOutput > 0 ? ((ok / totalOutput) * 100).toFixed(0) : 0;
