...
<head>
    ...
    <style>
        ...
        /* ✨ FIX SIDEBAR INDEPENDENT SCROLL */
        #accordionSidebar { 
        ...
        .nav-item .nav-link { 
            padding: 0.85rem 1.2rem !important; margin: 2px 12px; border-radius: 12px; 
            font-size: 0.7rem; font-weight: 700; color: rgba(255,255,255,0.55) !important; 
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); 
            display: flex; align-items: center;
        }
        .nav-item .nav-link i { font-size: 0.9rem; width: 1.5rem; transition: 0.3s; }
        .nav-item .nav-link:hover { color: #fff !important; background: rgba(255,255,255,0.08); transform: translateX(5px); }
        .nav-item.active .nav-link { background: var(--ind-blue) !important; color: #fff !important; box-shadow: 0 8px 15px rgba(67, 97, 238, 0.25); }
        
        .sidebar-heading { 
            color: rgba(255,255,255,0.25) !important; font-size: 0.6rem !important; 
            font-weight: 800; letter-spacing: 2.5px; margin-top: 1.5rem; margin-left: 22px; margin-bottom: 0.5rem;
        }

        /* Dropdown Style */
        .collapse-inner { background: rgba(255,255,255,0.03) !important; border-radius: 12px !important; margin: 2px 12px; padding: 0.4rem 0; border: 1px solid rgba(255,255,255,0.05); }
        .collapse-item { font-weight: 600 !important; font-size: 0.65rem !important; color: rgba(255,255,255,0.5) !important; padding: 0.6rem 1.2rem !important; border-radius: 8px; margin: 1px 8px; transition: 0.2s; }
        .collapse-item:hover { background: var(--ind-blue) !important; color: #fff !important; text-decoration: none; padding-left: 1.5rem !important; }

        /* ✨ TOPBAR GLASS */
        .topbar { background: var(--glass-white) !important; backdrop-filter: blur(15px); border-bottom: 1px solid var(--ind-border); height: 4.5rem; position: sticky; top: 0; z-index: 1000; }
        #clock-wrapper { background: var(--ind-navy); padding: 7px 16px; border-radius: 12px; border: 1.5px solid rgba(67, 97, 238, 0.5); display: flex; align-items: center; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
        #clock { font-family: 'JetBrains Mono', monospace; color: #fff; font-weight: 700; font-size: 0.85rem; letter-spacing: 1px; }

        @media (max-width: 768px) {
            #accordionSidebar { position: fixed; left: -250px; }
        ...
        .main-content-area { padding-top: 1.5rem; padding-bottom: 5rem; min-height: 85vh; }
    </style>
</head>
...
<body id="page-top">
    ...
    <div id="wrapper">
        ...
        <ul class="navbar-nav sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            {{-- 🗓️ 3. MANUFACTURING PLAN (STAMPING & WELDING) --}}
            ...
            <li class="nav-item">
                ...
                <div id="collapseWeldingPPIC" class="collapse {{ Request::is('ppic-welding*') ? 'show' : '' }}" data-parent="#accordionSidebar">
                    <div class="py-2 collapse-inner">
                        <a class="collapse-item text-primary" href="{{ route('ppic.welding.index') }}">Welding Dashboard</a>
                    ...
                    <ul class="navbar-nav ml-auto">
                        <li class="nav-item d-flex align-items-center">
                            ...
                            <form action="{{ route('logout') }}" method="POST" class="ml-3">
                                @csrf
                                <button type="submit" class="btn btn-sm btn-outline-danger rounded-pill px-3 font-weight-bold" style="font-size: 0.6rem;"><i class="fas fa-power-off"></i></button>
                            </form>
                        </li>
                    </ul>
                </nav>
